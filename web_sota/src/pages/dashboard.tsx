    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">
            Data from FastAPI at{" "}
            <code className="text-slate-300">{getApiBase()}</code> — not
            fabricated.
          </p>
        </div>
        <Button
      ...
      {error && (
        <div className="rounded-md border border-amber-900/80 bg-amber-950/40 px-4 py-3 text-sm text-amber-100">
          <strong className="text-amber-50">Backend unreachable.</strong>{" "}
          {error} — Start the API:{" "}
          <code className="rounded bg-slate-900 px-1">web_sota\start.ps1</code>{" "}
          (or uvicorn on the port in{" "}
